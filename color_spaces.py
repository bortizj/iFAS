...
def rgbToxyz(RGB):
    A = np.array([[42.62846,  38.29084,  13.67019], [21.64618,  72.06528,   5.83799], [1.77295,  12.93408,  92.75945]])
    X = A[0,0]*RGB[:,:,0]+A[0,1]*RGB[:,:,1]+A[0,2]*RGB[:,:,2]
    Y = A[1,0]*RGB[:,:,0]+A[1,1]*RGB[:,:,1]+A[1,2]*RGB[:,:,2]
    Z = A[2,0]*RGB[:,:,0]+A[2,1]*RGB[:,:,1]+A[2,2]*RGB[:,:,2]
...
def xyzToo123(XYZ):
    A = np.array([[0.2787,  0.7218, -0.1066], [-0.4488,  0.2898, -0.0772], [0.0860, -0.5900,  0.5011]])
...
def o123Toxyz(O123):
    A = np.array([[0.2787,  0.7218, -0.1066], [-0.4488,  0.2898, -0.0772], [0.0860, -0.5900,  0.5011]])
    A = np.linalg.inv(A)
    X = A[0, 0]*O123[:,:,0]+A[0,1]*O123[:,:,1]+A[0, 2]*O123[:,:,2]
    Y = A[1, 0]*O123[:,:,0]+A[1,1]*O123[:,:,1]+A[1, 2]*O123[:,:,2]
    Z = A[2, 0]*O123[:,:,0]+A[2,1]*O123[:,:,1]+A[2, 2]*O123[:,:,2]
    return np.dstack((X, Y, Z))
...
def colorfulness(RGB, sizewin = 3, type = 'Hasler'): #type = 'Hasler', 'Gao', or 'Panetta'
    ...
    if type == 'Gao':
        c = 0.02*np.log(sigma_alpha_sq/(np.power(np.abs(mu_alpha),0.2)+K)+K)*np.log(sigma_beta_sq/(np.power(np.abs(mu_beta),0.2)+K)+K)
        C = 0.02*np.log(Sigma_alpha_sq/(np.power(np.abs(Mu_alpha),0.2)+K)+K)*np.log(Sigma_beta_sq/(np.power(np.abs(Mu_beta),0.2)+K)+K)
    elif type == 'Panetta':
            ...
            Sigma_alpha_sq_c = signal.convolve2d(alpha*alpha, np.rot90(window/window.size,2), mode='valid') - Mu_gamma_sq
            Sigma_beta_sq_c  = signal.convolve2d(beta*beta, np.rot90(window/window.size,2), mode='valid') - Mu_gamma_sq
...
def l_alpha_beta(RGB):
    ...
    A = np.dot(np.array([[1/np.sqrt(3), 0, 0], [0, 1/np.sqrt(6), 0], [0, 0, 1/np.sqrt(2)]]),\
...
def rgb2icam(RGB):
    ...
    base_imgX, detail_imgX = MU.fastbilateralfilter(XYZimg[:,:, 0])
    base_imgY, detail_imgY = MU.fastbilateralfilter(XYZimg[:,:, 1])
    base_imgZ, detail_imgZ = MU.fastbilateralfilter(XYZimg[:,:, 2])
...
def iCAM06_blur(img, d):
    ...
    distMap = MU.idl_dist(Y.shape[0],Y.shape[1])
...
def scielab_simple(sampPerDeg, image):
    ...
    [k1, k2, k3] = MU.separableFilters(sampPerDeg, 3)
    p1 = MU.separableConv(opp[:,:,0], k1, np.abs(k1))
    p2 = MU.separableConv(opp[:,:,1], k2, np.abs(k2))
    p3 = MU.separableConv(opp[:,:,2], k3, np.abs(k3))
...
def colorfulness_Gao(alpha, beta, K=2):
    mu_alpha = np.mean(alpha)
    mu_beta = np.mean(beta)
    sigma_alpha_sq = np.var(alpha)
    sigma_beta_sq = np.var(beta)
...
def delta_HS(H_ref, H_pro, S_ref, S_pro):
    w1 = 0.3
    w2 = 0.1
    deltaH = np.abs(np.mean(H_ref) - np.mean(H_pro))
    deltaS = np.abs(np.mean(S_ref) - np.mean(S_pro))
    deltaHS = w1 * deltaH + w2 * deltaS
    return deltaHS


def deltaE2000(Lstd, astd, bstd, Lsample, asample, bsample, KLCH=np.array([1, 1, 1])):
    kl = KLCH[0]
    kc = KLCH[1]
    kh = KLCH[2]
...
def deltaEmahalanobis(Lstd, astd, bstd, Lsample, asample, bsample):
    Cabstd = np.sqrt(np.power(astd, 2) + np.power(bstd, 2))
    Cabsample = np.sqrt(np.power(asample, 2) + np.power(bsample, 2))
    Cabarithmean = (Cabstd + Cabsample) / 2.
    G = 0.5 * (1 - np.sqrt(np.power(Cabarithmean, 7) / (np.power(Cabarithmean, 7) + 25. ** 7)))
    ...
    hpstd = np.arctan2(bstd, apstd)
    hpstd = hpstd + 2 * np.pi * (hpstd < 0)
    hpstd[np.where((np.abs(apstd) + np.abs(bstd)) == 0)] = 0.
    hpsample = np.arctan2(bsample, apsample)
    hpsample = hpsample + 2 * np.pi * (hpsample < 0)
    hpsample[np.where((np.abs(apsample) + np.abs(bsample)) == 0)] = 0.
    num_1 = Lstd.size - 1.
    muL = np.mean(Lstd)
    muC = np.mean(Cabstd)
    muh = np.mean(hpstd)
    ...
    CDM = np.sqrt(A[0, 0] * np.power(deltaL, 2) + A[1, 1] * np.power(deltaC, 2) + A[2, 2] * np.power(deltaH, 2) +\
