...
def gen_data(x, a, fun_type='linear', noise=0, n_outliers=0, random_state=0, *arg):
    if fun_type == 'linear':
        y = a[0] + a[1] * x
        ...
        y = arg[1].predict(arg[0].transform(x))
    else:
        y = a[0] + a[1] * x
...
def optimize_function(x, y, fun_type='linear'):
    if fun_type == 'kNNR':
        return KnnRegression(x, y)
    else:
        fun = globals()[fun_type]
        res_soft_l1 = optimize.least_squares(fun, initial_gues(fun_type), loss='soft_l1', f_scale=0.1, args=(x, y))
        # returns the solution x or parameters of the function to optimize
        return res_soft_l1.x


def KnnRegression(X, y):
    # initializing the scaler and the model to be returned
    KnnR_scaler = preprocessing.StandardScaler()
    model = neighbors.KNeighborsRegressor(n_neighbors=5)
    # fittin the scaler to the training / input data
    KnnR_scaler.fit(X)
    # training the model
    trainedModel = model.fit(KnnR_scaler.transform(X), y)
    return [KnnR_scaler, trainedModel]
