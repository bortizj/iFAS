...
import numpy as np
from iFAS.gui.ifas_misc import convert_ifnan
...
def heat_map(pearson, spearman, ccd, tau):
    ...
    fig.canvas.manager.set_window_title("Correlation heat map plot")
...
def bar_plot(correlations, axes_labels=None, target_var_idx=""):
    ...
    fig.canvas.manager.set_window_title("Correlation bar plot with " + target_var_idx)
...
def box_plot(correlations, axes_labels=None, target_var_idx=""):
    ...
    width = np.round(1. / (len(axes_labels) + 1), 6)
    fig, acorrs = plt.subplots(1, 1)
    fig.canvas.manager.set_window_title("Correlation box plot with " + target_var_idx)
    acorrs.tick_params(labelsize=16)
