...
def cd01_SdeltaE2000(Ref_image, Pro_image):
    ...
    h1 = wi[0, 0] * MU.gaussian(xx, yy, si[0, 0]) + wi[0, 1] * MU.gaussian(xx, yy, si[0, 1]) + wi[0, 2] * MU.gaussian(
        xx, yy, si[0, 2])
    h1 = h1 / np.sum(h1[:])
    h2 = wi[1, 0] * MU.gaussian(xx, yy, si[1, 0]) + wi[1, 1] * MU.gaussian(xx, yy, si[1, 1])
    h2 = h2 / np.sum(h2[:])
    h3 = wi[2, 0] * MU.gaussian(xx, yy, si[2, 0]) + wi[2, 1] * MU.gaussian(xx, yy, si[2, 1])
    h3 = h3 / np.sum(h3[:])
    XYZ_ref = CAM.rgbToxyz(Ref_image / 255.)
    XYZ_pro = CAM.rgbToxyz(Pro_image / 255.)
    O123_ref = CAM.xyzToo123(XYZ_ref)
    O123_pro = CAM.xyzToo123(XYZ_pro)
    ...
    XYZ_ref = CAM.o123Toxyz(np.dstack((O1_ref, O2_ref, O3_ref)))
    XYZ_pro = CAM.o123Toxyz(np.dstack((O1_pro, O2_pro, O3_pro)))
    RGB_ref_back = 255 * CAM.xyzTorgb(XYZ_ref)
    RGB_ref_back[RGB_ref_back > 255] = 255
    RGB_ref_back[RGB_ref_back < 0] = 0
    RGB_ref_back = np.uint8(RGB_ref_back)
    RGB_pro_back = 255 * CAM.xyzTorgb(XYZ_pro)
...
def cd02_colordif_mahalanobis(Ref_image, Pro_image):
    Lab_ref = color.rgb2lab(Ref_image)
    Lab_pro = color.rgb2lab(Pro_image)
    Lstd = Lab_ref[:, :, 0]
    astd = Lab_ref[:, :, 1]
...
def cd03_colorfulness_diff_Gao(Ref_image, Pro_image, sizewin=3, type='Gao'):
    C_ref, c_ref = CAM.colorfulness(Ref_image, sizewin, type)
    C_pro, c_pro = CAM.colorfulness(Pro_image, sizewin, type)
    return np.abs(C_ref - C_pro), np.abs(c_ref - c_pro)


def cd04_color_SSIM(Ref_image, Pro_image):
    wL = 3.05
    walpha = 1.1
    wbeta = 0.85
    L_alpha_beta_ref = CAM.l_alpha_beta(Ref_image)
    L_alpha_beta_pro = CAM.l_alpha_beta(Pro_image)
    ssim_map_L, mssim_L = MU.SSIM(L_alpha_beta_ref[:, :, 0], L_alpha_beta_pro[:, :, 0])
    ssim_map_alpha, mssim_alpha = MU.SSIM(L_alpha_beta_ref[:, :, 1], L_alpha_beta_pro[:, :, 1])
    ssim_map_beta, mssim_beta = MU.SSIM(L_alpha_beta_ref[:, :, 2], L_alpha_beta_pro[:, :, 2])
...
def cd05_chroma_spread_extreme(Ref_image, Pro_image):
    ...
    mCb_ref, mCr_ref, MuCb_ref, MuCr_ref = MU.meanblock(YCbCr_ref[:, :, 1], YCbCr_ref[:, :, 2])
    mCb_pro, mCr_pro, MuCb_pro, MuCr_pro = MU.meanblock(YCbCr_pro[:, :, 1], YCbCr_pro[:, :, 2])
...
def cd06_colorhist_diff(Ref_image, Pro_image,blk_size=16):
    Lab_ref = color.rgb2lab(Ref_image)
    Lab_pro = color.rgb2lab(Pro_image)
    Diff = MU.block_proc(Lab_ref, blk_size, C=Lab_pro, funname='IhistogramintersectionLab')
    H_ref = MU.colorhistogram(Lab_ref, minc=np.array([0, -128, -128]), maxc=np.array([100, 127, 127]), nbins=8)
    H_pro = MU.colorhistogram(Lab_pro, minc=np.array([0, -128, -128]), maxc=np.array([100, 127, 127]), nbins=8)
...
def cd07_weighted_deltaE(Ref_image, Pro_image):
    ...
    hist_c, _ = np.histogram(180.*MU.convert_vec(hpsample)/np.pi, h_edges)
...
def cd08_cid_appearance(Ref_image, Pro_image):
    iCAM_ref = CAM.rgb2icam(Ref_image)#ipt_image
    iCAM_pro = CAM.rgb2icam(Pro_image)
...
def cd13_cid(Ref_image, Pro_image):
    ...
    Img2 = Pro_image
    CyclesPerDegree = 20
    Img1_XYZ = CAM.ImageSRGB2XYZ(Img1)
    Img1_filt = CAM.scielab_simple(2 * CyclesPerDegree, Img1_XYZ)
    Img1_LAB2000HL = CAM.ImageXYZ2LAB2000HL(Img1_filt)
    Img2_XYZ = CAM.ImageSRGB2XYZ(Img2)
    Img2_filt = CAM.scielab_simple(2 * CyclesPerDegree, Img2_XYZ)
    Img2_LAB2000HL = CAM.ImageXYZ2LAB2000HL(Img2_filt)
    Window = MU.matlab_style_gauss2D(shape=(11,11),sigma=1.5)
    img1 = Img1_LAB2000HL
    img2 = Img2_LAB2000HL
    L1 = img1[:,:, 0]
    A1 = img1[:,:, 1]
...
def cd14_circular_hue(Ref_image, Pro_image, mode='valid'):
    ...
    dC = np.mean(DC)
    DL, dL = MU.SSIM(Lab_ref[:,:,0], Lab_pro[:,:,0],mode=mode)
    DE = 1-DH*DC*DL
    dE = 1-dH*dC*dL
    return DE, dE
...
def cd15_osa_ucsdE(Ref_image, Pro_image):
    Ljg_ref = CAM.RGBToLjg(Ref_image)
    Ljg_pro = CAM.RGBToLjg(Pro_image)
...
def cd19_SSIMipt(Ref_image, Pro_image, mode='valid'):
    iCAM_ref = CAM.rgb2icam(Ref_image)  # ipt_image
    iCAM_pro = CAM.rgb2icam(Pro_image)
    ssim_map_i, mssim_i = MU.SSIM(iCAM_ref[:, :, 0], iCAM_pro[:, :, 0])
    ssim_map_p, mssim_p = MU.SSIM(iCAM_ref[:, :, 1], iCAM_pro[:, :, 1])
    ssim_map_t, mssim_t = MU.SSIM(iCAM_ref[:, :, 2], iCAM_pro[:, :, 2])
...
def cd20_cPSNRHA(Ref_image, Pro_image, wsize=11):
    ...
    DCb = PCb * CCbpro
    DCr = PCr * CCrpro
    _, p_hvs_m_YCHMA, _, YCHMA = MU.dct_block_mse(YCbCr_ref[:, :, 0], CYpro)
    _, p_hvs_m_CbCHMA, _, CbCHMA = MU.dct_block_mse(YCbCr_ref[:, :, 1], CCbpro)
    _, p_hvs_m_CrCHMA, _, CrCHMA = MU.dct_block_mse(YCbCr_ref[:, :, 2], CCrpro)
    _, p_hvs_m_YDHMA, _, YDHMA = MU.dct_block_mse(YCbCr_ref[:, :, 0], DY)
    _, p_hvs_m_CbDHMA, _, CbDHMA = MU.dct_block_mse(YCbCr_ref[:, :, 1], DCb)
    _, p_hvs_m_CrDHMA, _, CrDHMA = MU.dct_block_mse(YCbCr_ref[:, :, 2], DCr)
    if PY < 1:
        c = 0.002
    else:
        c = 0.25
    ...
    p_hvs_m_YCHMA = MU.clip_psnr(p_hvs_m_YCHMA)
    p_hvs_m_CbCHMA = MU.clip_psnr(p_hvs_m_CbCHMA)
    p_hvs_m_CrCHMA = MU.clip_psnr(p_hvs_m_CrCHMA)
...
def cd21_vsi(Ref_image, Pro_image):
    ...
    sigmaD = 145
    sigmaC = 0.001
    saliencyMap1 = MU.SDSP(image1, sigmaF, omega0, sigmaD, sigmaC)
    saliencyMap2 = MU.SDSP(image2, sigmaF, omega0, sigmaD, sigmaC)
...
def cd22_SSIM(Ref_image, Pro_image):
    Yref = MU.checkifRGB(Ref_image)
    Ypro = MU.checkifRGB(Pro_image)
    return MU.SSIM(Yref, Ypro)


def cd23_deltaEab(Ref_image, Pro_image):
    Lab_ref = color.rgb2lab(Ref_image)
    Lab_pro = color.rgb2lab(Pro_image)
    Lstd = Lab_ref[:, :, 0]
    astd = Lab_ref[:, :, 1]
...
#     DE00, de00 = cd10_deltaHS(image_ref, image_pro)
#     print de00
